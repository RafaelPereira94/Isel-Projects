...
%
% imdilate_demo.m
% Demonstração da operação de dilatação sobre imagens binárias.
%

function imdilate_demo(filename)
...
for radius=2 : 1 : 15
    ...
    % Aplicar a dilatação com o elemento estruturante atual.
    dilateBW = imdilate(originalBW,se);
    
    % Mostrar imagem original e após erosão.
    figure(1);
    subplot(121); imshow(originalBW); title('Original');
    subplot(122); imshow(dilateBW); title(['Dilation (Radius = ', num2str(radius) ' )']);
    pause(2)
